...
public class DeathLogServer implements DedicatedServerModInitializer {
    ...
    private static final DynamicCommandExceptionType NO_PLAYER_FOR_PROFILE = new DynamicCommandExceptionType(o -> Component.literal("Player " + ((NameAndId) o).name() + " is not online"));
    ...
    public void onInitializeServer() {
        ServerLifecycleEvents.SERVER_STARTING.register(server -> {
            storage = new ServerDeathLogStorage(server.registryAccess());
    ...
    private int executeView(CommandContext<CommandSourceStack> context, Function<List<DeathInfo>, Integer> indexProvider, Function<Integer, CommandSyntaxException> exceptionProvider) throws CommandSyntaxException {
        ServerPlayer player = context.getSource().getPlayerOrException();
        NameAndId profile = getProfile(context);
        List<DeathInfo> deathInfoList = storage.getDeathInfoList(profile.id());
    ...
    private static Predicate<CommandSourceStack> hasPermission(String node) {
        return DeathLogCommon.usePermissions() ? Permissions.require(node, 4) : Commands.hasPermission(Commands.LEVEL_OWNERS);
    }

    public static boolean hasPermission(ServerPlayer player, String node) {

        return DeathLogCommon.usePermissions() ? Permissions.check(player, node, 4) : player.permissions().hasPermission(net.minecraft.server.permissions.Permissions.COMMANDS_OWNER);
    }
    ...
    private static RequiredArgumentBuilder<CommandSourceStack, GameProfileArgument.Result> createProfileArgument() {
        return argument("player", GameProfileArgument.gameProfile()).suggests((context, builder) -> {
            ...
            return SharedSuggestionProvider.suggest(playerManager.getPlayers().stream().map(Player::getScoreboardName), builder);
        });
    }
